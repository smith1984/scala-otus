...
object WalletTransferApp extends IOApp.Simple {
  ...
  final class InMemWallet[F[_]](ref: Ref[F, BigDecimal]) extends Wallet[F] {
    ...
    def withdraw(amount: BigDecimal): F[Either[WalletError, Unit]] = ref.modify(balanceCurrent =>
      if (balanceCurrent < amount) (balanceCurrent, Left(BalanceTooLow))
      else (balanceCurrent - amount, Right(()))
    )
  }
